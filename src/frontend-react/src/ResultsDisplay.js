...
// A small helper component to render a single User
const UserResult = ({ user }) => (
    <div className="result-item">
        <div className="item-header">
            <span className="item-title">{user.FirstName} {user.LastName}</span>
        </div>
        <div className="item-company">{user.ProfessionalTitle || 'N/A'}</div>
    </div>
);
...
const ResultsDisplay = ({ rawGql, rawJson }) => {
  ...
  if (resultData) {
      ...
      } else if (resultData.createUser || resultData.createJob) {
          const item = resultData.createUser || resultData.createJob;
          resultsContent = <p>✅ Successfully created: {item.FirstName || item.title}</p>;
      }
      // Add more parsers here for other operations like 'apply', 'updateUser', etc.
  }
  
  // Fallback for successful operations that don't return a list
  if (!resultsContent) {
